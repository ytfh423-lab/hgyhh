...
// RefundTopUp 标记充值单退款成功并回退用户额度
func RefundTopUp(tradeNo string) error {
	if tradeNo == "" {
		return errors.New("未提供订单号")
	}
	...
	if quotaToDeduct > 0 {
		RecordLog(userId, LogTypeTopup, fmt.Sprintf("LinuxDO 充值退款成功，回退额度: %v，支付金额：%f", logger.FormatQuota(quotaToDeduct), payMoney))
	}
	return nil
}
