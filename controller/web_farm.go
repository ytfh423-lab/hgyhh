...
func SetAdminFishOverride(tgId, fishKey string) {
	...
	defer adminFishOverrideMu.Unlock()
	adminFishOverride[tgId] = fishKey
}

func ConsumeAdminFishOverride(tgId string) (string, bool) {
	adminFishOverrideMu.Lock()
	defer adminFishOverrideMu.Unlock()
	key, ok := adminFishOverride[tgId]
	if ok {
		delete(adminFishOverride, tgId)
	}
	return key, ok
}

// WebFarmFishSetNext (admin only) sets the next fish for a user
func WebFarmFishSetNext(c *gin.Context) {
	var req struct {
		UserId  int    `json:"user_id"`
		FishKey string `json:"fish_key"`
	}
	if err := c.ShouldBindJSON(&req); err != nil || req.UserId <= 0 || req.FishKey == "" {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "参数错误：需要 user_id 和 fish_key"})
		return
	}
	if fishTypeMap[req.FishKey] == nil {
		var validKeys []string
		for _, ft := range fishTypes {
			validKeys = append(validKeys, ft.Key)
		}
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "无效的鱼种 key", "valid_keys": validKeys})
		return
	}
	...
	if tgId == "" {
		tgId = fmt.Sprintf("u_%d", user.Id)
	}

	fish := fishTypeMap[req.FishKey]
	SetAdminFishOverride(tgId, req.FishKey)
	adminId := c.GetInt("id")
	common.SysLog(fmt.Sprintf("Admin %d set next fish for user %d (%s): %s %s [%s]", adminId, req.UserId, tgId, fish.Emoji, fish.Name, fish.Rarity))

	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": fmt.Sprintf("已设置用户 %d 下一杆必钓: %s %s [%s]", req.UserId, fish.Emoji, fish.Name, fish.Rarity),
	})
}

// WebFarmFishTypes returns all fish type keys for admin reference
func WebFarmFishTypes(c *gin.Context) {
	var types []gin.H
	for _, ft := range fishTypes {
...
func farmSellToAdmin(user *model.User, tgId string, itemType string, itemKey string, quantity int, baseValue int, description string) farmSellResult {
	...
	if adminId > 0 && adminId != user.Id {
		...
		model.AddFarmLog(tgId, "sell", finalValue, fmt.Sprintf("[出售]%s: $%.2f(加成+%d%%)", description, webFarmQuotaFloat(finalValue), bonus))

		adminTgId := getAdminFarmId(adminId)
		if adminTgId != "" {
			if itemType == "crop" || itemType == "warehouse" {
				_ = model.AddToWarehouse(adminTgId, itemKey, quantity)
			} else if itemType == "fish" {
				_ = model.IncrementFarmItem(adminTgId, "fish_"+itemKey, quantity)
			}
			model.AddFarmLog(adminTgId, "admin_buy", -finalValue, fmt.Sprintf("[收购]来自%s: %s ×%d", tgId, description, quantity))
		}

		return farmSellResult{BaseValue: baseValue, PrestigeBonus: prestigeAdd, FinalValue: finalValue, AdminSell: true}
	}

	_ = model.IncreaseUserQuota(user.Id, finalValue, true)
	model.AddFarmLog(tgId, "sell", finalValue, fmt.Sprintf("[出售]%s: $%.2f(加成+%d%%)", description, webFarmQuotaFloat(finalValue), bonus))
	return farmSellResult{BaseValue: baseValue, PrestigeBonus: prestigeAdd, FinalValue: finalValue, AdminSell: false}
}

func getAdminFarmId(adminUserId int) string {
	admin, err := model.GetUserById(adminUserId, false)
	if err != nil {
		return ""
	}
	if admin.TelegramId != "" {
	...
	return fmt.Sprintf("u_%d", admin.Id)
}
...
type webPlotInfo struct {
	...
	ProtectionRemain int64   `json:"protection_remain"`
}
...
func buildPlotInfo(plot *model.TgFarmPlot) webPlotInfo {
	...
	case 1:
		...
		if crop != nil {
			...
			info.ReadyAt = plot.PlantedAt + growSecs
		}
	...
	case 2:
		...
		if crop != nil {
			...
			if plot.MaturedAt > 0 {
				protEnd := plot.MaturedAt + int64(stealCfg.OwnerProtectionMinutes)*60
				if now < protEnd {
...
func WebFarmSellSeed(c *gin.Context) {
	...
	sellResult := farmSellToAdmin(user, tgId, "crop", cropKey, req.Quantity, refund, fmt.Sprintf("出售%s%s种子×%d", crop.Emoji, crop.Name, req.Quantity))

	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": fmt.Sprintf("出售 %s%s种子 ×%d，获得 $%.2f", crop.Emoji, crop.Name, req.Quantity, webFarmQuotaFloat(sellResult.FinalValue)),
	})
}
...
func WebFarmSteal(c *gin.Context) {
	...
	// 获取可偷地块
	plots, err := model.GetStealablePlotsV2(req.VictimId)
...
func WebFarmMarket(c *gin.Context) {
	...
	type categoryGroup struct {
		...
		AvgMult  int         `json:"avg_multiplier"`
		HotItem  string      `json:"hot_item"`
	...
	for _, cm := range catMeta {
		items, ok := catItems[cm.Key]
		if !ok || len(items) == 0 {
			continue
		}
		totalMult, hotMult, coldMult := 0, 0, 999
		hotName, coldName := "", ""
		for _, it := range items {
			totalMult += it.Multiplier
			if it.Multiplier > hotMult {
				hotMult = it.Multiplier
				hotName = it.Emoji + it.Name
			}
			if it.Multiplier < coldMult {
				coldMult = it.Multiplier
				coldName = it.Emoji + it.Name
			}
		}
...
func WebFarmFishDo(c *gin.Context) {
	...
	if overrideKey, ok := ConsumeAdminFishOverride(tgId); ok {
		if fd := fishTypeMap[overrideKey]; fd != nil {
			fish = fd
	...
	specialEffect := ""
	if fish.Key == "goldendragon" {
		specialEffect = "golden_dragon"
...
func WebFarmFishSell(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": fmt.Sprintf("卖出 %d 条鱼，收入 $%.2f（含市场波动+转生加成）", totalCount, webFarmQuotaFloat(sellResult.FinalValue)),
		"data": gin.H{
			"count": totalCount,
			"total": webFarmQuotaFloat(sellResult.FinalValue),
		},
	})
}
...
func WebFarmWarehouseSell(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": fmt.Sprintf("出售成功！获得 $%.2f", webFarmQuotaFloat(sellResult.FinalValue)),
		"data": gin.H{
			"name":     name,
			"quantity": item.Quantity,
			"earned":   webFarmQuotaFloat(sellResult.FinalValue),
		},
	})
}
...
func WebFarmWarehouseSellAll(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": fmt.Sprintf("全部出售成功！获得 $%.2f", webFarmQuotaFloat(sellResult.FinalValue)),
		"data": gin.H{
			"earned": webFarmQuotaFloat(sellResult.FinalValue),
		},
	})
