...
func StartEntrustCleanupTask() {
	go func() {
		for {
			time.Sleep(5 * time.Minute)
			cleanupExpiredEntrustsAndRefund()
		}
	}()
}

func cleanupExpiredEntrustsAndRefund() {
	expired := model.CleanupExpiredEntrusts()
	if expired > 0 {
		common.SysLog(fmt.Sprintf("Entrust cleanup: marked %d tasks as expired", expired))
	}

	tasks, err := model.GetExpiredUnrefundedEntrusts()
	...
	for _, task := range tasks {
		...
		_ = model.MarkEntrustEscrowRefunded(task.Id)
		common.SysLog(fmt.Sprintf("Entrust #%d expired: settled workers and refunded remaining escrow to owner %s", task.Id, task.OwnerTelegramId))
	}
}
