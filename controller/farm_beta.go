...
func FarmBetaStatus(c *gin.Context) {
	...
	betaExpired := false
	betaEndTime := common.FarmBetaEndTime
	if betaEndTime != "" {
		endT, err := time.Parse(time.RFC3339, betaEndTime)
		if err == nil && time.Now().After(endT) {
			betaExpired = true
		}
	}
...
func CheckFarmBetaAccess() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		// Check if beta period has expired using FarmBetaEndTime
		betaEndTimeStr := common.FarmBetaEndTime
		if betaEndTimeStr != "" {
			betaEnd, err := time.Parse(time.RFC3339, betaEndTimeStr)
			if err == nil && time.Now().After(betaEnd) {
