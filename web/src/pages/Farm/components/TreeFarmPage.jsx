...
import { API, formatBalance, formatDuration, showError, confirmAction } from './utils';
              ...
              {detailSlot.status === 1 && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                    ...
                    <Text type='tertiary'>
                      {detailSlot.progress}% · {formatDuration(detailSlot.remaining)}
                      {detailSlot.ready_at > 0 && (
                        <span style={{ marginLeft: 4, opacity: 0.7 }}>
                          ({new Date(detailSlot.ready_at * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })})
                        </span>
                      )}
                    </Text>
                  </div>
