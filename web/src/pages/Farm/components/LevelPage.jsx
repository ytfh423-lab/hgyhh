...
import { API, showError, showSuccess, confirmAction } from './utils';
...
const LevelPage = ({ actionLoading, loadFarm, t }) => {
  ...
  const doLevelUp = async () => {
    const price = lvData?.upgrade_cost != null ? `$${lvData.upgrade_cost.toFixed(2)}` : '';
    if (!await confirmAction(t('升级确认'), `${t('确认花费')} ${price} ${t('升级到')} Lv.${(lvData?.level || 1) + 1}？`)) return;
    setLoading(true);
    try {
