...
/* ═══════════════════════════════════════════════════════════════
   1. HorseRaceGame — 赛马 🏇
   疯狂按空格加速，但有体力条，按太快体力耗尽会减速
   ═══════════════════════════════════════════════════════════════ */
export const HorseRaceGame = ({ game, onComplete, t }) => {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.code === 'Space') {
        ...
        else if (phase === 'playing' && stateRef.current) {
          const s = stateRef.current;
          if (s.player.stamina > 2) {
            s.player.speed = Math.min(s.player.speed + 0.35, 1.2);
            s.player.stamina = Math.max(0, s.player.stamina - 3.5);
          }
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Player physics
      s.player.stamina = Math.min(100, s.player.stamina + 0.25);
      const staminaFactor = s.player.stamina < 15 ? 0.3 : s.player.stamina < 40 ? 0.7 : 1;
      s.player.speed *= 0.92;
      s.player.x += s.player.speed * staminaFactor * 0.4;
      s.player.sprite = (s.player.sprite + s.player.speed * 0.3) % 2;

      // NPC physics
      for (const npc of s.npcs) {
        const wobble = Math.sin(s.tick * 0.03 + npc.wobble) * 0.04;
        npc.x += (npc.speed + wobble) * 0.4;
      }

      // Check finish
      const allHorses = [
        { name: '🏇 ' + t('你'), x: s.player.x, isPlayer: true },
        ...s.npcs.map(n => ({ name: n.name, x: n.x, isPlayer: false })),
      ];
      const done = allHorses.filter(h => h.x >= FINISH);
      if (done.length > 0) {
        allHorses.sort((a, b) => b.x - a.x);
        s.rankings = allHorses;
        s.finished = true;
        const playerRank = allHorses.findIndex(h => h.isPlayer) + 1;
        const score = playerRank === 1 ? 100 : playerRank === 2 ? 65 : playerRank === 3 ? 30 : 0;
        setResult({ rank: playerRank, score });
        setPhase('done');
      ...
      // Finish line
      const fx = (FINISH / TRACK) * W;
      ctx.fillStyle = '#facc15';
      for (let y = 0; y < H; y += 10) {
        ctx.fillRect(fx, y, 3, 5);
      }
      // Horses
      const drawHorse = (x, lane, emoji, color, spritePhase) => {
        const px = (x / TRACK) * W;
        const py = lane * laneH + laneH / 2;
        ctx.font = '28px serif';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        const bounce = Math.sin(spritePhase * Math.PI) * 3;
        ctx.fillText(emoji, px, py + bounce);
        // Name plate
        ctx.fillStyle = color;
        ctx.fillRect(px - 16, py - 22, 32, 12);
        ctx.fillStyle = '#fff';
      ...
      ctx.textAlign = 'left';
      ctx.fillText('⚡ ' + Math.round(s.player.stamina) + '%', 14, H - 14);
  ...
  if (phase === 'ready') {
    return (
      <div className='farm-gc-ready'>
        ...
        <div className='farm-gc-ready-desc'>⚡ {t('疯狂按空格加速')} — {t('但别耗尽体力')}!</div>
        <div className='farm-gc-ready-hint'>{t('体力低于15%会大幅减速')} · {t('按空格/点击开始')}</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, width: '100%' }}>
      <div className='farm-gc' style={{ aspectRatio: '370/220' }}>
        <canvas ref={canvasRef} />
      </div>
      <div style={{ fontSize: 12, color: 'var(--farm-text-3)' }}>
        {t('按空格/点击屏幕加速')} · {t('控制体力节奏')}
      </div>
      <Button theme='solid' size='default' className='farm-btn'
        style={{ fontWeight: 700, minWidth: 120 }}
        onMouseDown={() => {
          if (stateRef.current && stateRef.current.player.stamina > 2) {
            stateRef.current.player.speed = Math.min(stateRef.current.player.speed + 0.35, 1.2);
            stateRef.current.player.stamina = Math.max(0, stateRef.current.player.stamina - 3.5);
          }
        }}>
        🏇 {t('加速')}!
      </Button>
