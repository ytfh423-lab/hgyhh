...
const FishGodTool = ({ t }) => {
  ...
  useEffect(() => {
    API.get('/api/tgbot/farm/fish/types').then(({ data }) => {
      if (data.success) setFishTypes(data.data || []);
    }).catch(() => {});
  }, []);

  const handleSetNext = async () => {
    if (!userId || !selectedFish) {
      showError(t('请填写用户ID和选择鱼种'));
      return;
    }
    setLoading(true);
    try {
      const { data: res } = await API.post('/api/tgbot/farm/fish/set-next', {
        user_id: parseInt(userId),
        fish_key: selectedFish,
      });
  ...
  return (
    <div style={{ marginBottom: 16, padding: 12, border: '1px solid var(--semi-color-border)', borderRadius: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
        <Typography.Text style={{ width: 80 }}>{t('用户ID')}:</Typography.Text>
        <Input value={userId} onChange={setUserId} placeholder='123' style={{ width: 100 }} />
        <Typography.Text style={{ width: 60 }}>{t('鱼种')}:</Typography.Text>
        <select
          value={selectedFish}
          onChange={(e) => setSelectedFish(e.target.value)}
          style={{ padding: '4px 8px', borderRadius: 4, border: '1px solid var(--semi-color-border)', background: 'var(--semi-color-bg-1)', color: 'var(--semi-color-text-0)' }}
        >
          <option value=''>{t('选择鱼种')}</option>
          {fishTypes.map((f) => (
            <option key={f.key} value={f.key}>{f.emoji} {f.name} [{f.rarity}]</option>
          ))}
        </select>
        <Button onClick={handleSetNext} loading={loading} theme='solid' size='small'>
          {t('设置下一杆')}
        </Button>
      </div>
      <Typography.Text type='tertiary' size='small'>
        {t('设置后该用户下一次钓鱼必定钓到指定鱼种，仅生效一次。')}
      </Typography.Text>
    </div>
  );
};
...
const TgBotPage = () => {
  ...
  return (
    <div className='mt-[60px] px-2'>
      <Tabs type='button' style={{ marginBottom: 16 }} lazyRender keepDOM={false}>
        <TabPane tab={t('机器人设置')} itemKey='bot'>
      ...
      <Card className='!rounded-2xl' style={cardStyle}>
        ...
        <TabPane tab={t('农场参数')} itemKey='farm'>
      <div className='space-y-4'>
      {/* ===== 农场游戏设置 ===== */}
      <Card
        title={
      ...
      >
        <div className='mb-4'>
          ...
          <Typography.Title heading={6} style={{ marginTop: 16, marginBottom: 12 }}>🎣 {t('管理员钓鱼神器')}</Typography.Title>
          <FishGodTool t={t} />
          ...
          <Typography.Title heading={6} style={{ marginTop: 8, marginBottom: 8 }}>{t('转生价格')}</Typography.Title>
          <div style={{ marginBottom: 12, padding: 12, border: '1px solid var(--semi-color-border)', borderRadius: 8 }}>
            <Typography.Text>{t('转生价格使用固定公式，无需配置：')}</Typography.Text>
            <ul style={{ margin: '8px 0', paddingLeft: 20, lineHeight: 1.8 }}>
              <li>{t('首次转生：$200')}</li>
              <li>{t('第2~100次：每次递增10%')}</li>
              <li>{t('第101次以后：每次递增50%')}</li>
            </ul>
          </div>
        ...
        <TabPane tab={t('公告与群组')} itemKey='announce'>
      <div className='space-y-4'>
      {/* ===== 农场公告设置 ===== */}
      <Card
      ...
      >
        ...
        <TabPane tab={t('用户与管理')} itemKey='admin'>
      <div className='space-y-4'>
      {/* ===== 农场活跃用户 ===== */}
      <Card
        title={
